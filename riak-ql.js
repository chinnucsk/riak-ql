...
(function() {
  ...
  // add trim to String's prototype, missing in spidermonkey
  String.prototype.trim = function () {
    return this.replace(/^\s*/, "").replace(/\s*$/, "");
  }

  var _rql = function(expr, obj) {
  
    var parts = expr.split('where ');
  
    if (parts.length > 2) throw new Error("Syntax Error: Too many 'where's in: " + expr);
  
    var main = parts[0].trim() || ':root',
      result = true;
    ...
    if (parts[1]) {
      ...
      var handle = function(elem) {
        if (typeof elem == 'string') {
          var negate = elem[0] === '!';
          if (negate) elem = elem.slice(1);
        
          elem = selector.match(elem, obj).filter(function(result) {
    ...
    if (!!result) {
      return selector.match(main, obj);
    } else {
      return [];
    }    
  ...
  query = function(values, keydata, arg) {
